...
enum layers { BASE1, MOD2, MOD3, MOD4 };
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [BASE1] = LAYOUT(
        ...
        MO(MOD2),        KC_A,    KC_S,    KC_D,    KC_F,    KC_G,    KC_H,    KC_J,    KC_K,    KC_L,    KC_SCLN,   KC_QUOT, KC_ENT,              KC_PGUP,
        KC_LSFT,           KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,    KC_N,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH, KC_RSFT,         KC_UP,    KC_PGDN,
        KC_LCTL,   KC_LGUI,   KC_LALT,      KC_SPC,          MO(MOD3),        SPC_SFT,       KC_RALT,  MO(MOD4),  KC_RCTL,   KC_LEFT, KC_DOWN,   KC_RGHT
    ),
    [MOD2] = LAYOUT(
    ...
    [MOD4] = LAYOUT(
        ...
        RGB_TOG, RGB_SAD, RGB_SAI, RGB_HUD, RGB_HUI, XXXXXXX, RGB_RMOD, RGB_MOD, XXXXXXX, RGB_SPD, RGB_SPI, RGB_VAD, RGB_VAI,      AUTO_HUE,     KC_PAUSE,
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    ...
    // shift hue
    if (!auto_hue){
        uint8_t time = scale16by8(g_rgb_timer, qadd8(rgb_matrix_config.speed / 7, 1));
        hsv_secondary.h = 255 - time;
    }
    else {
...
#define CSRIC_TIME_LIMIT 50
#define CSRIC_TIME_HEATUP_DIV 5
#define CSRIC_TIME_SPEED 20
//#define CSRIC_APPLY_LIMIT 60
// pressed key special effect
void HOLDING_HORIZONTAL(HSV hsv_secondary_max_v){
   uint8_t apply_limit = rgb_matrix_get_hsv().v;
   ...
   for (uint8_t i = 0; i < RGB_MATRIX_LED_COUNT; ++i) {
      ...
      for (uint8_t j = 0; j < captured_count; ++j) {
         ...
         uint8_t dx =
             (g_led_config.point[i].x > g_led_config.point[hold_pos].x)
                 ? g_led_config.point[i].x - g_led_config.point[hold_pos].x
                 : g_led_config.point[hold_pos].x - g_led_config.point[i].x;
         uint8_t dy =
             (g_led_config.point[i].y > g_led_config.point[hold_pos].y)
                 ? g_led_config.point[i].y - g_led_config.point[hold_pos].y
                 : g_led_config.point[hold_pos].y - g_led_config.point[i].y;

         uint16_t tick = scale16by8(pos_to_tick_l[hold_pos], CSRIC_TIME_SPEED);
         // attack
         if (tick <= CSRIC_TIME_LIMIT / CSRIC_TIME_HEATUP_DIV) {
            tick = (CSRIC_TIME_LIMIT - tick * CSRIC_TIME_HEATUP_DIV) * 2;
         } else {
            tick = tick - CSRIC_TIME_LIMIT / CSRIC_TIME_HEATUP_DIV;
            // max out tick.
            if (tick > CSRIC_TIME_LIMIT) {
               tick = CSRIC_TIME_LIMIT;
            }
         }

         uint16_t dist = dx + dy * 2;
         // prune to a cross shape. I like it.
         if (dx > 15 && dy > 10) {
            dist = 255;
         } else if (dx > 10 && dy > 10) {
            dist = qadd8(dist, 40);
         ...
         if (effect > 255 - apply_limit) {
            effect = 255;
         }
         if (effect < 255) {
            if (dx > hsv_buffer_s_change_value_max) {
               hsv_buffer_s_change_value_max = dx;
            }
            if (dy > hsv_buffer_s_change_value_max) {
               hsv_buffer_s_change_value_max = dy;
            }
            int8_t light_increament;
            if (effect < CSRIC_HOLDING_LIMIT) {
               light_increament = 240;
            } else if (effect - CSRIC_HOLDING_LIMIT > 250 / 2) {
               light_increament = 0;
            } else {
               light_increament =
      ...
      hsv_buffer.h += hsv_buffer_s_change_value_max / 2;
      // apply
      if (hsv_buffer.v >= apply_limit) {
